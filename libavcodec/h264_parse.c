                             ...
                             int err_recognition, void *logctx)
{
    ...
    if (data[0] == 1) {
        ...
        *nal_length_size = (data[4] & 0x03) + 1;
        ...
        // Skip any avcC extension bytes (chroma_format, bit_depth, SPS ext)
        // by scanning forward for the mvcC configurationVersion == 1.
        while (p - data < size && p[0] != 1)
            p++;
        if (p - data < size && p[0] == 1 && size - (p - data) >= 7) {
            const uint8_t *mvc = p;
