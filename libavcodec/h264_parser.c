...
//    mb_addr= pc->mb_addr - 1;
    ...
    for (i = 0; i < buf_size; i++) {
        ...
        if (state == 7) {
            ...
            /* MVC combined packets: skip dep-view boundary detection
             * after a base slice has been found. */
                                  ...
                                  const uint8_t * const buf, int buf_size)
{
    ...
    for (;;) {
        ...
        /* MVC: skip dep-view slice NALs to prevent POC corruption */
        if (nal.type == H264_NAL_PREFIX || nal.type == H264_NAL_EXTEN_SLICE)
            continue;
        ...
        case H264_NAL_SUB_SPS:
            /* MVC subset SPS: register the embedded SPS so that the
             * dependent view's PPS (which references its id) parses
             * without error; trailing MVC extension bits are ignored. */
            ff_h264_decode_seq_parameter_set(&nal.gb, avctx, &p->ps, 1);
            break;
        case H264_NAL_PPS:
