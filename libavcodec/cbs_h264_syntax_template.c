...
static int FUNC(sps_mvc_applicable_op)(CodedBitstreamContext *ctx, RWContext *rw,
                                       H264RawSPSMVCApplicableOp *current)
{
    int err, k;

    ub(3, applicable_op_temporal_id);
    ue(applicable_op_num_target_views_minus1, 0, H264_MVC_MAX_VIEWS - 1);
    for (k = 0; k <= current->applicable_op_num_target_views_minus1; k++)
        ues(applicable_op_target_view_id[k], 0, 1023, 1, k);
    ue(applicable_op_num_views_minus1, 0, H264_MVC_MAX_VIEWS - 1);

    return 0;
}

static int FUNC(sps_mvc_level_value)(CodedBitstreamContext *ctx, RWContext *rw,
                                     H264RawSPSMVCLevelValue *current)
{
    int err, j;

    ub(8, level_idc);
    ue(num_applicable_ops_minus1, 0, H264_MAX_APPLICABLE_OPS - 1);

    for (j = 0; j <= current->num_applicable_ops_minus1; j++)
        CHECK(FUNC(sps_mvc_applicable_op)(ctx, rw, &current->applicable_ops[j]));
                                   ...
                                   H264RawSPSMVCExtension *current)
{
    ...
    ue(num_views_minus1, 0, 1023);
    if (current->num_views_minus1 > H264_MVC_MAX_VIEWS - 1) {
        av_log(ctx->log_ctx, AV_LOG_VERBOSE,
               "Only stereo MVC is supported (stream has %d views).\n",
               current->num_views_minus1 + 1);
    ...
    for (i = 0; i <= current->num_level_values_signalled_minus1; i++)
        CHECK(FUNC(sps_mvc_level_value)(ctx, rw, &current->level_values[i]));
                            ...
                            H264RawSubsetSPS *current)
{
    ...
    if (current->sps.profile_idc == 118 ||
        ...
        if (current->mvc_vui_parameters_present_flag) {
            /* mvc_vui_parameters_extension() is not implemented; leave
             * the unit undecomposed so that the write path passes the
             * original bits through instead of truncating the SPS. */
            av_log(ctx->log_ctx, AV_LOG_VERBOSE,
                   "MVC VUI parameters not supported.\n");
            return AVERROR(EAGAIN);
        }
    ...
    if (current->additional_extension2_flag) {
        /* additional_extension2_data_flag until the end of the RBSP;
         * not representable, keep the unit undecomposed. */
        av_log(ctx->log_ctx, AV_LOG_VERBOSE,
               "Subset SPS additional extension data not supported.\n");
        return AVERROR(EAGAIN);
    }
                     ...
                     H264RawPPS *current)
{
    ...
    /* Per H.7.4.1.2.1 subset SPSes use a separate seq_parameter_set_id
     * value space from SPSes; whether a PPS refers to one or the other
     * is only known from the slice NAL type using the PPS.  Real MVC
     * streams use disjoint ids, so a regular-SPS-first lookup picks the
                              ...
                              H264RawSliceHeader *current)
{
    ...
    /* MVC extension slices belong to the dependent view, whose SPS is
     * the subset SPS (separate id value space, cf. the PPS parser). */
    sps = h264->sps[pps->seq_parameter_set_id];
    if (h264->subset_sps[pps->seq_parameter_set_id] &&
        (!sps ||
         current->nal_unit_header.nal_unit_type == H264_NAL_EXTEN_SLICE))
                                 ...
                                 H264RawPrefixNALUnit *current)
{
    ...
    /* For MVC (svc_extension_flag == 0) the prefix NAL unit RBSP is
     * empty (H.7.3.2.12); the MVC extension header has already been
     * parsed as part of the NAL unit header, and there are no
     * rbsp_trailing_bits.  SVC prefix NAL units are not supported. */
