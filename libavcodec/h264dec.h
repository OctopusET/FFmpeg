 ...
 * Set on the base picture at the in-band base-to-dep view transition
 * (h264_slice.c) and cleared whenever mvc_base_pic is repointed or
 * dropped (view transition back, dep drain, flush).  Prevents
 * MMCO/sliding window from freeing the base view picture while dep
 * view slices still need it.
 */
...
#define FRAME_RECOVERED_HEURISTIC  (1 << 2)
     ...
     *
     * Architecture: per-view DPB with view_id tagging.
     *
     * Each view has its own DPB (views[0].DPB[] for base,
     * views[1].DPB[] for dep) and short_ref[]/long_ref[] lists.
     * Each H264Picture is tagged with view_id so ref list construction
     * (h264_refs.c) picks refs from the matching view's DPB.
    ...
    /* DTS of the packet currently being decoded.  The receive_frame API
     * (unlike the legacy decode() callback) does not auto-stamp pkt_dts on
     * output frames in decode.c, so we track it here and apply it in
     * output_frame(), mirroring HEVCContext.pkt_dts (hevc/hevcdec.h). */
    int64_t pkt_dts;
    ...
    int mvc_hwaccel_warned;         ///< log-once flag for hwaccel dep-view skip
     ...
     */
    AVFifo *mvc_pending_pkts;       ///< AVPacket fifo, lazily allocated
    size_t  mvc_last_dep_size;      ///< size of last extracted dep side-data (dedup)
    uint8_t mvc_last_dep_sig[32];   ///< first 32 bytes of last extracted dep (dedup)
    int     mvc_warned_mt;          ///< 1 after logging "frame-mt disables dep view"
    H264Picture *mvc_pending_output_pic; ///< base view output saved across view transition

    /*
     * MVC output sort buffer.  Because base and dep views have independent
     * reorder queues with potentially asymmetric delay (e.g. interlaced
     * PAFF), frames can be pushed to output_fifo in an order different from
     * the expected base-before-dep within the same access unit.  We drain
     * output_fifo into this buffer, then emit in (pts, view_id) ascending
     * order so downstream muxers/filters see stable pairing.
     */
#define H264_MVC_SORT_BUF_SIZE 16
    AVFrame *mvc_sort_buf[H264_MVC_SORT_BUF_SIZE];
    int mvc_sort_buf_count;

    /* Per-view display-order counter [0]=base, [1]=dep.  Raw streams carry no
     * container timestamps (pts == AV_NOPTS_VALUE); this synthesizes a pairing
     * key so the (pts, view_id) merge above can still align base[k] with
     * dep[k] -- the two views of the same access unit get the same index. */
    int64_t mvc_disp_seq[2];
