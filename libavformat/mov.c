 ...
 * The mvcC has the same avcC-style layout.  It is appended verbatim
 * after the avcC; the decoder's extradata parser handles both blocks
 * so it sees both views' parameter sets.
 */
static int mov_read_mvcc(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (atom.size < 7 || (uint64_t)atom.size > (1 << 30))
        return AVERROR_INVALIDDATA;
    ...
    if (st->codecpar->extradata_size >
        INT_MAX - mvc_size - AV_INPUT_BUFFER_PADDING_SIZE)
        return AVERROR_INVALIDDATA;
